...
contract Escrow {
     ...
     */
    function order(string memory _name, uint256 _amount) external payable {
    ...
    function complete(string memory _name) external {
        ...
        for (uint256 i = 0; i < item.orders.length; i++) {
            if (
                item.orders[i].buyer == msg.sender &&
                item.orders[i].status == Status.ORDER
            ) {
                item.orders[i].status = Status.COMPLETE;
                payable(item.seller).transfer(
                    item.orders[i].ordered * item.price
                );
                found = true;
                break;
            }
        ...
        if (!found)
            revert("Item not ordered or in different status than ORDER");
    }
    ...
    function complain(string memory _name) external {
        ...
        for (uint256 i = 0; i < item.orders.length; i++) {
            if (
                ...
                payable(item.orders[i].buyer).transfer(
                    item.orders[i].ordered * item.price
                );
                found = true;
                break;
     ...
     */
    function getOrder(string memory _name)
        external
        view
        returns (uint256 _ordered, Status _status)
    {
